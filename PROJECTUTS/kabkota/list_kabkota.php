...
// Buat kondisi filter dan pencarian
$conditions = [];
$params = [];

if (!empty($provinsi_id)) {
    $conditions[] = "k.provinsi_id = :provinsi_id";
    $params[':provinsi_id'] = $provinsi_id;
}

if (!empty($search)) {
    $conditions[] = "(k.nama LIKE :search OR k.kode LIKE :search)";
...
$count_query = "SELECT COUNT(*) FROM kabkota k $where_clause";
...
// Query untuk mengambil data dengan pagination, filter, dan pencarian
$query = "SELECT k.*, p.nama as provinsi_nama 
          FROM kabkota k 
...
$stmt->execute();
$kabkota_data = $stmt->fetchAll();

// Ambil data provinsi untuk filter
    ...
    <div class="sidebar">
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" role="menu">
          ...
          <li class="nav-item"><a href="list_kabkota.php" class="nav-link active"><i class="nav-icon fas fa-city"></i><p>Data Kabupaten/Kota</p></a></li>
              ...
              <div class="row">
                ...
                <div class="col-md-4">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="search" name="search" placeholder="Cari nama atau kode..." value="<?= htmlspecialchars($search) ?>">
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    ...
                    <div>
                      ...
                      <a href="list_kabkota.php" class="btn btn-default">Reset</a>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="card">
          ...
          <div class="card-body table-responsive p-0">
            <table class="table table-hover text-nowrap">
              ...
              <tbody>
                <?php if (count($kabkota_data) > 0): ?>
                  <?php $no = $start + 1; foreach ($kabkota_data as $row): ?>
                    <tr>
                      <td><?= $no++ ?></td>
          ...
          <div class="card-footer clearfix">
            ...
            <ul class="pagination pagination-sm m-0 float-right">
              <?php if ($page > 1): ?>
                <li class="page-item"><a class="page-link" href="?page=1<?= $provinsi_id ? "&provinsi_id=$provinsi_id" : "" ?><?= $search ? "&search=$search" : "" ?>">&laquo;</a></li>
              <?php endif; ?>
              ...
              for ($i = $start_page; $i <= $end_page; $i++): ?>
                <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                  <a class="page-link" href="?page=<?= $i ?><?= $provinsi_id ? "&provinsi_id=$provinsi_id" : "" ?><?= $search ? "&search=$search" : "" ?>"><?= $i ?></a>
                </li>
              <?php endfor; ?>
              
              <?php if ($page < $total_pages): ?>
                <li class="page-item"><a class="page-link" href="?page=<?= $total_pages ?><?= $provinsi_id ? "&provinsi_id=$provinsi_id" : "" ?><?= $search ? "&search=$search" : "" ?>">&raquo;</a></li>
              <?php endif; ?>
            </ul>
          </div>
        </div>
...
$(document).ready(function() {
  ...
  // Auto submit form saat provinsi berubah
